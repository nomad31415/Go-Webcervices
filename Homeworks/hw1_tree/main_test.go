...
func TestTreeFull(t *testing.T) {
	...
	result := out.String()
	if len(result) != len(testFullResult) {
		t.Errorf("test for OK Failed - size not match Actual:%d Expected:%v\n", len(result) , len(testFullResult))
	}
...
func TestTreeDir(t *testing.T) {
	...
	if len(result) != len(testDirResult) {
		t.Errorf("test for OK Failed - size not match Actual:%d Expected:%v\n", len(result) , len(testDirResult))
	}
